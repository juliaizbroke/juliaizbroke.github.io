...
function App() {
  ...
  const addItem = () => {
    ...
    const newItem = {
      ...
      discount: discRef.current.value
   };
    const isExist = dataItems.findIndex((v) => v.item === item.name && v.ppu === newItem.ppu);
    if(isExist !== -1) {
      dataItems[isExist].qty = parseInt(dataItems[isExist].qty) + parseInt(qtyRef.current.value);
        ...
        <Col md={4} style={{ backgroundColor: "#e4e4e4" }}>
          ...
          <Row>
            <Col>
              ...
              <Form.Control type="number" ref={discRef} defaultValue={0} />
            </Col>
          </Row>
          <hr />
